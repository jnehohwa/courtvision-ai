...
from courtvision.e2e_server import prepare_database, reset_redis_database
...
def test_e2e_server_refuses_redis_reset_outside_e2e(monkeypatch):
    monkeypatch.setattr(settings, "environment", "production")
...
def test_e2e_server_runs_worker_when_enabled(monkeypatch):
    class FakeWorkerProcess:
        def __init__(self) -> None:
            ...
            self.killed = False
            self.stdout = io.StringIO('{"event":"replay_worker_ready"}\n')
    ...
    monkeypatch.setattr(e2e_server, "reset_redis_database", lambda: None)
    ...
    assert popen_calls[0][1]["stdout"] == e2e_server.subprocess.PIPE
    assert popen_calls[0][1]["stderr"] == e2e_server.subprocess.STDOUT
